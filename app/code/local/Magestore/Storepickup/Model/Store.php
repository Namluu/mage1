...
//        }
        ...
        if ((version_compare(Mage::getVersion(), '1.13', '>=')) && (version_compare(Mage::getVersion(), '1.14.0.1', '<='))) {
           ...
           $urlrewrite = $this->loadByIdpath($url_key, Mage::app()->getStore()->getStoreId());
           
                $urlrewrite->setData('identifier',$url_key);
                $urlrewrite->setData('entity_type', 1);
                $urlrewrite->setData('is_system', 1);
                $urlrewrite->setData('request_path','storepickup/'. $url_key);
            ...
            $urlrewrite = $this->loadByIdpath($url_key, Mage::app()->getStore()->getStoreId());
                ...
                $urlrewrite->setData('request_path','storepickup/'. $url_key);
                $urlrewrite->setData('target_path', 'storepickup/index/index/viewstore/' . $id);
                $urlrewrite->setData('store_id', Mage::app()->getStore()->getStoreId());
        }else{
